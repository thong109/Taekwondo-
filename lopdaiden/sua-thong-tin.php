<?php require_once __DIR__. '/autoload/autoload.php';
  $data = $db->fetchID("giaovien",$_SESSION['giaovien_id']);
  ...
  $data2 =
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        if(empty($error))
        {     
            $isset = $db->fetchOne("giaovien","email = '".$data2['email']."' AND password = '".MD5(postInput('oldpassword'))."' ");
            if($isset > 0)
            {
                ...
                $id_update = $db->update("giaovien",$data2,array("email" => $data2['email']));
                if($id_update>0)
                {
